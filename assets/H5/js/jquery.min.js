...
$.ajaxSetup({
  ...
  error: function (result) {
    console.log(result);
    //alert('加载出错！' + JSON.stringify(result));
  },
  complete: function (xhr, TS) {
    if (xhr.status == 500 || xhr.status > 500) {
      alert('程序错误，错误代码：' + xhr.status + '，错误原因：' + xhr.responseText);
    } else if (xhr.status == 0 || xhr.status == 101) {
      alert('请求超时，请检查您的网络情况。');
    } else if (xhr.status == 401) {
      alert('拒绝访问，请检查是否登陆或者登陆是否超时！');
      window.app && window.app.onAppSessionTimeout && window.app.onAppSessionTimeout();
    } else if (xhr.status == 403) {
      alert('拒绝访问，当前用户没有权限！');
      window.app && window.app.onAppCancel && window.app.onAppCancel();
    } else if (xhr.status == 404) {
      alert('无法找到所请求的资源');
      // window.location.href = "../App/AppLogin?LastUrl=" + window.location.href.split('#')[0] + "";
...
function setBar(opts) {
  if (window.app && window.app.onAppSetNaviBar) {
    opts = typeof opts == 'string' ? opts : JSON.stringify(opts);
    window.app.onAppSetNaviBar(opts);
  } else {
    document.title = opts.title + (opts.subTitle || '');
    console.log(opts);
  }
}
